...
const createListing = wrapAsync(async (req, res) => {
  ...
  moderateListing(listing._id).catch((err) => {
    console.error(
      `Unexpected error in moderateListing for ${listing._id}:`,
      err,
    );
  });
});
...
const getListings = wrapAsync(async (req, res) => {
  ...
  // Update:::: Wildcard(*${search}*) added instead of fixed ${search} so searching for "book" lists both book category and vivobook.
  if (search) {
    pipeline.unshift({
      $search: {
        index: "default",
  ...
  res.status(200).json({
    ...
    pagination: {
      ...
      nextCursor:
        listings.length > 0 ? listings[listings.length - 1]._id : null,
    },
  });
});
...
const getListingById = wrapAsync(async (req, res) => {
  const listing = await Listing.findById(req.params.id)
    .populate(
      "seller",
      "name username email contactInfo isContactDisplayable year department",
    )
  ...
  if (listing.status !== "Listed") {
    const isOwner =
      req.user && listing.seller._id.toString() === req.user._id.toString();
    if (!isOwner) {
      return res.status(404).json({ message: "Listing not found" });
    }
  }
...
const updateListing = wrapAsync(async (req, res) => {
  ...
  res
    .status(200)
    .json({ message: "Listing updated and resubmitted for review", listing });
